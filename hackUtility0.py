...
def hide_statusbar():
	...
	if can_run_script:
		time.sleep(1)
		print("Trying to hide the status bar.")
		time.sleep(2)
		print('Code loaded! Status bar is now hidden.\n')
		ObjCClass('UIApplication').sharedApplication().statusBar().setHidden_(1)
	else:
		time.sleep(1)
		print('Script is not running. Please type: start_script() to load it.')
	
def show_statusbar():
	global can_run_script
	if can_run_script:
		time.sleep(1)
		print("Trying to show back the status bar.")
		time.sleep(2)
...
def change_statusbar_color(color):
	...
	if can_run_script:
		time.sleep(1)
		print('Trying hard to change status bar color, to your chosen one.')
		time.sleep(4)
		if color == 'black':
...
def mess_up_resolution():
	...
	if can_run_script:
		time.sleep(1)
		print("Trying to mess up the phone's Resolution.")
		time.sleep(2)
...
def fix_resoltion():
	...
	if can_run_script:
		time.sleep(1)
		print("Trying to fix the phone's Resolution")
		time.sleep(2)
...
def help_me(help_for=''):
	...
	if can_run_script:
		if help_for == '':
			print("For help on a specific command type: help_me(start_script()) or help_me('change_statusbar_color(color), etc.\n\nFor help on all commands type: help_me('everything')\n\n")
		elif help_for == 'everything':
			print("1. help_me() provides help and guidance for the commands.\n2. start_script() will start or re-start the execution of this utilitty when exited using exit() command.\n3. hide_statusbar() will hide the statusbar inside Pythonista App.\n4. show_statusbar() will reveal the hidden status bar.\n5. change_starusbar_color(color) command will change the statusbar color inside Pythonista App. Example: change_statusbar_color('red'). Available colors: black, white, red, blue, green, gray, orange, purple, yellow.\n6. mess_up_resolution() will dramatically reduce phone's resolution within Pythonista App.\n7. fix_resolution() will increase phone's resolution back to normal, ouf!\n8. exit() will quit the utility and you will have to use start_script() to re-run it.\n")
		elif help_for == 'start_sciprt()':
			print("start_script() will start or re-start the execution of this utilitty when exited using exit() command.\n")
		elif help_for == 'hide_starusbar':
			print("hide_statusbar() will hide the statusbar inside Pythonista App.")
		elif help_for == 'show_statusbar':
			print("show_statusbar() will reveal the hidden status bar.")
		elif help_for == 'change_statusbar_color(color)':
			print("change_starusbar_color(color) command will change the statusbar color inside Pythonista App. Example: change_statusbar_color('red'). Available colors: black, white, red, blue, green, gray, orange, purple, yellow.")
		elif help_for == 'mess_up_resolution()':
			print("mess_up_resolution() will dramatically reduce phone's resolution within Pythonista App.")
		elif help_for == 'fix_resoltion()':
			print("fix_resolution() will increase phone's resolution back to normal, ouf!")
		elif help_for == 'quit_script()':
			print("exit() will quit the utility and you will have to use start_script() to re-run it.")
		else:
			print('Wrong command.')
	else:
		time.sleep(1)
